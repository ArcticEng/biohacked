...
function VerifyInner() {
  ...
  if (status === "verifying") return <Loading msg="Verifying email" />;

  return (
    <Card style={{ textAlign: "center", padding: "40px 24px" }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>{status === "success" ? "✅" : "❌"}</div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: C.t1, marginBottom: 8 }}>{status === "success" ? "Email verified" : "Verification failed"}</h2>
      <p style={{ color: C.t3, fontSize: 13 }}>{status === "success" ? "Your account is now fully activated." : "This link is invalid or has already been used."}</p>
